...
- **Modules** can be instantiated. Think of it as the ato equivalent of a class.
...
**Important:** Always use toleranced values for parameters. If you constrain a `resistor.resistance` to `10kohm` there won't be a single part found because that's a tolerance of 0%.
...
- `resistor0.unnamed[0] ~ resistor1.unnamed[0]` connects two resistors in series.
...
- Preferably use `Electrical` interface for electrical interfaces.
...
- To explicitly pick a part for a module use `lcsc = "<LCSC_PART_NUMBER>"`.
